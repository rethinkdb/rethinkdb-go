...
func (suite *TimesConstructorsSuite) TestCases() {
...

	{
		...
		runAndAssert(suite.Suite, expected_, r.EpochTime(1.444).ToISO8601(), suite.session, r.RunOpts{
			GeometryFormat: "raw",
			GroupFormat:    "map",
		})
...

	{
		...
		runAndAssert(suite.Suite, expected_, r.EpochTime(1.001).ToISO8601(), suite.session, r.RunOpts{
			GeometryFormat: "raw",
			GroupFormat:    "map",
		})
...

	{
		...
		runAndAssert(suite.Suite, expected_, r.EpochTime(2.058).ToISO8601(), suite.session, r.RunOpts{
			GeometryFormat: "raw",
			GroupFormat:    "map",
		})
