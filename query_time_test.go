...
func (s *RethinkSuite) TestTimeTime(c *test.C) {
	...
	err := Time(1986, 11, 3, 12, 30, 15, "Z").RunRow(conn).Scan(&response)
...
func (s *RethinkSuite) TestTimeEpochTime(c *test.C) {
	var response time.Time
	err := EpochTime(531360000).RunRow(conn).Scan(&response)
	c.Assert(err, test.IsNil)
...
func (s *RethinkSuite) TestTimeISO8601(c *test.C) {
	...
	err := ISO8601("1986-11-03T08:30:00-07:00").RunRow(conn).Scan(&t1)
	c.Assert(err, test.IsNil)
...
func (s *RethinkSuite) TestTimeBetween(c *test.C) {
	...
	}).Count().RunRow(conn).Scan(&response)
...
func (s *RethinkSuite) TestTimeYear(c *test.C) {
	...
	err := Time(1986, 12, 3, 12, 30, 15, "Z").Year().RunRow(conn).Scan(&response)
...
func (s *RethinkSuite) TestTimeMonth(c *test.C) {
	...
	err := Time(1986, 12, 3, 12, 30, 15, "Z").Month().Eq(December()).RunRow(conn).Scan(&response)
...
func (s *RethinkSuite) TestTimeDay(c *test.C) {
	...
	err := Time(1986, 12, 3, 12, 30, 15, "Z").Day().Eq(Wednesday()).RunRow(conn).Scan(&response)
